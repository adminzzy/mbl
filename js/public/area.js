...
function change1(v){
    var str="0";
    for(i=0;i<v;i++){
        str+=("_"+(document.getElementById(sa[i]).selectedIndex-1));
    };
    var ss=document.getElementById(sa[v]);
    with(ss){
        length = 0;
        options[0]=new Option(opt0[v],opt0[v]);
        if(v && document.getElementById(sa[v-1]).selectedIndex>0 || !v){
            if(dsy.Exists(str)){
                ar = dsy.Items[str];
                for(i=0;i<ar.length;i++){
                    options[length]=new Option(ar[i],ar[i]);
...
var sa=["s_province1","s_city1","s_county1"];
var opt0 = ["省份","地级市","区/县"];//初始值
function _init_area(s_province,s_city,s_county){  //初始化函数
	//var s =[];
	//s.unshift(s_county);
	//s.unshift(s_city);
	//s.unshift(s_province);
	for(i=0;i<s.length-1;i++){
	  document.getElementById(s[i]).onchange=new Function("change("+(i+1)+")");
	}
	change(0);
}
function _init_area1(s_province,s_city,s_county){  //初始化函数
	//var s =[];
	//s.unshift(s_county);
	//s.unshift(s_city);
	//s.unshift(s_province);
	for(i=0;i<sa.length-1;i++){
	  document.getElementById(sa[i]).onchange=new Function("change1("+(i+1)+")");
	}
	change1(0);
}
